...
const DEFAULT_FAUCET_URL =
  import.meta.env.VITE_LINERA_FAUCET_URL || "http://localhost:8080";

const DEFAULT_APPLICATION_ID = import.meta.env.VITE_LINERA_APPLICATION_ID || "";
...
const LineraContextProvider = ({ children }) => {
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      ...
      const nextCurrentTurn = data?.currentTurn ?? null;
      setCurrentTurn((prev) => (Object.is(prev, nextCurrentTurn) ? prev : nextCurrentTurn));
  ...
  const resignMatch = useCallback(async () => {
    await gql(`mutation { resignMatch }`);
    await refresh();
  }, [gql, refresh]);

  const endGame = useCallback(
    async (status) => {
      await gql(`mutation { endGame(status: ${status}) }`);
      await refresh();
    },
    [gql, refresh]
  );
